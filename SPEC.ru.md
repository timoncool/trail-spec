...
**Статус:** Черновик
**Авторы:** timoncool
**Предыдущая:** [v1](https://github.com/timoncool/trail-spec/tree/v1)
...
- **v1** (предыдущая): плоский JSONL, 5 полей (`v`, `t`, `cid`, `act`, `req`), короткие имена
- **v2** (эта спека): читаемые имена, корреляция трейсов, причинность, стоимость, discovery, ротация

### Правила совместимости

- Серверы ДОЛЖНЫ записывать `version: 2`
- Серверы ДОЛЖНЫ уметь читать v1 записи (маппинг `v`→`version`, `t`→`timestamp`, `cid`→`content_id`, `act`→`action`, `req`→`requester`, `d`→`details`)
...
Миграция не нужна. v2 сервер читает оба формата прозрачно. Новые записи пишутся в v2.
...
**В: Почему имена полей изменились с v1 на v2?**
О: Протокол на десятилетия должен быть самодокументируемым. `content_id` понятен сразу; `cid` требует спеки. Накладные расходы ничтожны.

**В: v1 ещё поддерживается?**
