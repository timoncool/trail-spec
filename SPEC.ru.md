...
- **Путь по умолчанию:** `data/trail.jsonl` относительно корня MCP-сервера
- **Переопределение:** Серверы МОГУТ разрешать переопределение через переменную окружения `TRAIL_PATH` или параметр конструктора. Когда задано, значение используется как полный путь к JSONL-файлу.
...
|----------|-----|-----------|----------|
| `content_id` | `string` | — | Фильтр по content ID. Точное совпадение, или префикс заканчивающийся на `:` (например, `civitai:image:` найдёт все картинки с Civitai, но `civitai:image:1` найдёт только этот конкретный ID). |
...
Инструменты публикации (например, `send_photo`, `publish_post`) ДОЛЖНЫ принимать опциональные параметры TRAIL-контекста. Когда они переданы, инструмент автоматически дописывает запись `posted` при успехе или `failed` при ошибке.

### Передача контекста (параметр `_trail`)

Для стандартизации передачи TRAIL-контекста в инструменты публикации серверы ДОЛЖНЫ принимать опциональный параметр `_trail` — единый объект со всеми trail-полями:
...
- Параметр `_trail` ОПЦИОНАЛЕН. Если отсутствует, trail-запись не создаётся.
- Префикс подчёркивания (`_trail`) сигнализирует, что это метаданные протокола, а не параметр инструмента.
- `_trail.content_id` и `_trail.requester` ОБЯЗАТЕЛЬНЫ внутри `_trail`. `trace_id`, `caused_by` и `tags` опциональны.
- При успехе сервер дописывает trail-запись с `action: "posted"` и автоматически включает платформенные детали (например, `platform_id`, `url`).
- При ошибке сервер дописывает trail-запись с `action: "failed"` и включает структурированные детали ошибки.
- Серверы МОГУТ также принимать `content_id`, `requester`, `trace_id` как отдельные параметры верхнего уровня для обратной совместимости. При наличии и `_trail`, и отдельных параметров `_trail` имеет приоритет.
...
Текущая версия — `2`. Поле `version` обеспечивает эволюцию. Будущие версии могут добавить новые опциональные поля, новые стандартные действия или новые форматы кодирования.

### Версионирование спецификации

Спецификация следует семантическому версионированию (`MAJOR.MINOR.PATCH`):

- **PATCH** (например, 2.1.0 → 2.1.1): Исправление опечаток, уточнения. Без изменений схемы.
- **MINOR** (например, 2.1 → 2.2): Новые опциональные поля, новые стандартные действия, новые подполя details. Всегда обратно совместимо — парсер v2.1 ДОЛЖЕН корректно читать записи v2.2.
- **MAJOR** (например, 2.x → 3.0): Ломающие изменения обязательных полей или основной семантики. Поле `version` в записях увеличивается соответственно.

### Правила совместимости

- Серверы ДОЛЖНЫ записывать `version: 2` по этой спецификации
- Серверы ДОЛЖНЫ читать записи с `version` >= 2: извлекать известные поля, игнорировать неизвестные, НЕ отклонять запись
- Серверы МОГУТ логировать предупреждение при встрече `version` выше реализованного
- Серверы ДОЛЖНЫ игнорировать неизвестные поля записей на любом уровне (расширяемость)
- Неизвестные подполя `details` ДОЛЖНЫ сохраняться, не удаляться
- Парсер, написанный для спеки 2.x, ДОЛЖЕН корректно читать любую запись 2.y (где y > x)

### Политика устаревания

- Deprecated-фичи анонсируются минимум за одну минорную версию до удаления
- Удалённые фичи перечисляются в CHANGELOG
- Поле НЕ МОЖЕТ быть удалено в рамках одной мажорной версии после получения статуса `stable`
